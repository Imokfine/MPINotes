...
int main(int argc, char** argv)
{
    ...
    for (it = 0; it < maxit; it++) {
        ...
        exchangi2d(a, nx, ny, coords, xs, xe, ys, ye, MPI_COMM_WORLD, nbrleft, nbrright, nbrup, nbrdown);
        sweep2d(a, f, nx, xs, xe, ys, ye, b);

        exchangi2d(b, nx, ny, coords, xs, xe, ys, ye, MPI_COMM_WORLD, nbrleft, nbrright, nbrup, nbrdown);
...
void write_grid2d(double a[][maxn], int nx, int* xs, int* xe, int* ys, int* ye, int rank) {
    ...
    char filename[50];
    sprintf(filename, "2dnb_Grid_of_rank_%d.txt", rank);
