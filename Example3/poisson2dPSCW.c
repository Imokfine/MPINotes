...
int main(int argc, char** argv)
{
    ...
    // get all processors from MPI_COMM_WORLD
    MPI_Group world_group;
    MPI_Comm_group(MPI_COMM_WORLD, &world_group);

    // new groups
    MPI_Group origin[2], target[2];


    if (nbrright != MPI_PROC_NULL) {
        int ranks[1] = { nbrright };
        MPI_Group_incl(world_group, 1, ranks, &origin[0]);
    }
    if (nbrleft != MPI_PROC_NULL) {
        int ranks[1] = { nbrleft };
        MPI_Group_incl(world_group, 1, ranks, &target[0]);
    }
    if (nbrup != MPI_PROC_NULL) {
        int ranks[1] = { nbrup };
        MPI_Group_incl(world_group, 1, ranks, &origin[1]);
    }
    if (nbrdown != MPI_PROC_NULL) {
        int ranks[1] = { nbrdown };
        MPI_Group_incl(world_group, 1, ranks, &target[1]);
    }
    ...
    for (it = 0; it < maxit; it++) {
        ...
        exchangrma_2d_pscw(a, ny, xs, xe, ys, ye, wina, nbrleft, nbrright, nbrup, nbrdown, origin, target);
        sweep2d(a, f, nx, xs, xe, ys, ye, b);



        exchangrma_2d_pscw(b, ny, xs, xe, ys, ye, winb, nbrleft, nbrright, nbrup, nbrdown, origin, target);
...
void write_grid2d(double a[][maxn], int nx, int* xs, int* xe, int* ys, int* ye, int rank) {
    ...
    char filename[50];
    sprintf(filename, "PSCW_2d_Grid_of_rank_%d.txt", rank);
