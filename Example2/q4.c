...
int main(int argc, char** argv)
{
	...
	exchang3_2d(a, nx, ny, nprocs, dims, coords, xs, xe, ys, ye, myid, MPI_COMM_WORLD, nbrleft, nbrright, nbrup, nbrdown);
        sweep2d(a, f, nx, xs, xe, ys, ye, b);



	exchang3_2d(b, nx, ny, nprocs, dims, coords, xs, xe, ys, ye, myid, MPI_COMM_WORLD, nbrleft, nbrright, nbrup, nbrdown);
        sweep2d(b, f, nx, xs, xe, ys, ye, a);

        ldiff = griddiff2d(a, b, nx, xs, xe, ys, ye);
...
/* Inplemention of write_grid2d() and GatherGrid2d() */
...
void write_grid2d(double a[][maxn], int nx, int *xs, int *xe, int *ys, int *ye, int rank) {
    ...
    char filename[50];
    sprintf(filename, "2d_Grid_of_rank_%d.txt", rank);
